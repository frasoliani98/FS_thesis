...
import pandas as pd
import itertools
from important_functions import run_EAD_IKr, detect_EAD, baseline_run, plot_GA, baseline_EAD, baseline_run, run_EAD_ICal, baseline_EAD_ICalenha
import itertools
...
for i in block:
    ...
    # TOR BASELINE
    tbb,vbb = baseline_EAD_ICalenha(i)
    info, result = detect_EAD(tbb,vbb)


    mdp = min(vbb)
    max_p = max(vbb)
    max_p_idx = np.argmax(vbb)
    apa = max_p - mdp
    dvdt_max = np.max(np.diff(vbb[0:30])/np.diff(tbb[0:30]))    
        
    repol_pot = max_p - apa * 90/100
    idx_apd = np.argmin(np.abs(vbb[max_p_idx:] - repol_pot))
    apd_val = tbb[idx_apd+max_p_idx]
    apd_val_tor.append(apd_val)
    ...
    # IND1
    t1,v1 = run_EAD_ICal(ind_1, i, IC=None)
    ...
    # IND 2
    t2,v2 = run_EAD_ICal(ind_2, i, IC=None)
    ...
    # IND 3
    t3,v3 = run_EAD_ICal(ind_3, i, IC=None)
    ...
    repol_pot = max_p - apa * 90/100
    idx_apd = np.argmin(np.abs(v3[max_p_idx:] - repol_pot))
    ...
    # IND 4
    t4,v4 = run_EAD_ICal(ind_4, i, IC=None)
    ...
    # IND 5
    t5,v5 = run_EAD_ICal(ind_5, i, IC=None)
    ...
    # IND 6
    t6,v6 = run_EAD_ICal(ind_6, i, IC=None)
    ...
    # IND 7
    t7,v7 = run_EAD_ICal(ind_7, i, IC=None)
    ...
    # IND 8
    t8,v8 = run_EAD_ICal(ind_8, i, IC=None)
...
df7.to_excel('FS_ICaL_enha.xlsx', sheet_name='Sheet1')