...
def run_EAD(ind):
    ...
    mod['multipliers']['i_cal_pca_multiplier'].set_rhs(
        8*mod['multipliers']['i_cal_pca_multiplier'].value())
...
def run_EAD_ICal(ind, block, IC):

    ## EAD CHALLENGE: Istim = -.1
    mod, proto = get_ind_data(ind)
    
    mod['multipliers']['i_cal_pca_multiplier'].set_rhs(
        block*mod['multipliers']['i_cal_pca_multiplier'].value())
...
def baseline_EAD_ICalenha(enha):
    mod, proto, x = myokit.load('./tor_ord_endo2.mmt')
    mod['multipliers']['i_cal_pca_multiplier'].set_rhs(
...
def baseline_EAD_CaL():
    ...
    dat = sim.run(50000)
    #caib = dat['intracellular_ions.cai']
    #kib = dat['intracellular_ions.ki']
    tcb = dat['engine.time']
    #Ical = dat['ICal.ICal']
    IKr = dat['IKr.IKr']
    IK1 = dat['IK1.IK1']
    ...
    return t,v,tcb,IKr,IK1,IKb,IKs,INa,INaCa,INaK,INaL,INab
